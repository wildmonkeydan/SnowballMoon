...
bool input_GetButtonPressed(GameInput button, int id) {
	...
	case GI_ATTACK:
		return IsGamepadButtonPressed(id, GAMEPAD_BUTTON_RIGHT_FACE_DOWN) || IsKeyPressed(KEY_Z);
	}
}
...
bool input_GetButton(GameInput button, int id) {
	...
	case GI_ATTACK:
		return IsGamepadButtonDown(id, GAMEPAD_BUTTON_RIGHT_FACE_DOWN) || IsKeyDown(KEY_Z);
	}
}