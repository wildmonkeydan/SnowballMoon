...
JOY_SHOCK_API int JslConnectDevices();
JOY_SHOCK_API int JslGetConnectedDeviceHandles(int* deviceHandleArray, int size);
JOY_SHOCK_API void JslDisconnectAndDisposeAll();
JOY_SHOCK_API bool JslStillConnected(int deviceId);
...
JOY_SHOCK_API JOY_SHOCK_STATE JslGetSimpleState(int deviceId);
JOY_SHOCK_API IMU_STATE JslGetIMUState(int deviceId);
JOY_SHOCK_API MOTION_STATE JslGetMotionState(int deviceId);
JOY_SHOCK_API TOUCH_STATE JslGetTouchState(int deviceId, bool previousfalse);
JOY_SHOCK_API bool JslGetTouchpadDimension(int deviceId, int* sizeX, int* sizeY);

JOY_SHOCK_API int JslGetButtons(int deviceId);

// get thumbsticks
JOY_SHOCK_API float JslGetLeftX(int deviceId);
JOY_SHOCK_API float JslGetLeftY(int deviceId);
JOY_SHOCK_API float JslGetRightX(int deviceId);
JOY_SHOCK_API float JslGetRightY(int deviceId);
...
JOY_SHOCK_API float JslGetLeftTrigger(int deviceId);
JOY_SHOCK_API float JslGetRightTrigger(int deviceId);

// get gyro
JOY_SHOCK_API float JslGetGyroX(int deviceId);
JOY_SHOCK_API float JslGetGyroY(int deviceId);
JOY_SHOCK_API float JslGetGyroZ(int deviceId);

// get accumulated average gyro since this function was last called or last flushed values
JOY_SHOCK_API void JslGetAndFlushAccumulatedGyro(int deviceId, float* gyroX, float* gyroY, float* gyroZ);
...
JOY_SHOCK_API void JslSetGyroSpace(int deviceId, int gyroSpace);

// get accelerometor
JOY_SHOCK_API float JslGetAccelX(int deviceId);
JOY_SHOCK_API float JslGetAccelY(int deviceId);
JOY_SHOCK_API float JslGetAccelZ(int deviceId);

// get touchpad
JOY_SHOCK_API int JslGetTouchId(int deviceId, bool secondTouch);
JOY_SHOCK_API bool JslGetTouchDown(int deviceId, bool secondTouch);

JOY_SHOCK_API float JslGetTouchX(int deviceId, bool secondTouch);
JOY_SHOCK_API float JslGetTouchY(int deviceId, bool secondTouch);

// analog parameters have different resolutions depending on device
JOY_SHOCK_API float JslGetStickStep(int deviceId);
JOY_SHOCK_API float JslGetTriggerStep(int deviceId);
JOY_SHOCK_API float JslGetPollRate(int deviceId);
JOY_SHOCK_API float JslGetTimeSinceLastUpdate(int deviceId);

// calibration
JOY_SHOCK_API void JslResetContinuousCalibration(int deviceId);
JOY_SHOCK_API void JslStartContinuousCalibration(int deviceId);
JOY_SHOCK_API void JslPauseContinuousCalibration(int deviceId);
JOY_SHOCK_API void JslSetAutomaticCalibration(int deviceId, bool enabled);
JOY_SHOCK_API void JslGetCalibrationOffset(int deviceId, float* xOffset, float* yOffset, float* zOffset);
JOY_SHOCK_API void JslSetCalibrationOffset(int deviceId, float xOffset, float yOffset, float zOffset);
JOY_SHOCK_API JSL_AUTO_CALIBRATION JslGetAutoCalibrationStatus(int deviceId);

// this function will get called for each input event from each controller
JOY_SHOCK_API void JslSetCallback(void(*callback)(int, JOY_SHOCK_STATE, JOY_SHOCK_STATE, IMU_STATE, IMU_STATE, float));
// this function will get called for each input event, even if touch data didn't update
JOY_SHOCK_API void JslSetTouchCallback(void(*callback)(int, TOUCH_STATE, TOUCH_STATE, float));
// this function will get called for each device when it is newly connected
JOY_SHOCK_API void JslSetConnectCallback(void(*callback)(int));
// this function will get called for each device when it is disconnected
JOY_SHOCK_API void JslSetDisconnectCallback(void(*callback)(int, bool));

// super-getter for reading a whole lot of state at once
JOY_SHOCK_API JSL_SETTINGS JslGetControllerInfoAndSettings(int deviceId);
// what kind of controller is this?
JOY_SHOCK_API int JslGetControllerType(int deviceId);
// is this a left, right, or full controller?
JOY_SHOCK_API int JslGetControllerSplitType(int deviceId);
// what colour is the controller (not all controllers support this; those that don't will report white)
JOY_SHOCK_API int JslGetControllerColour(int deviceId);
...
JOY_SHOCK_API void JslSetLightColour(int deviceId, int colour);
// set controller rumble
JOY_SHOCK_API void JslSetRumble(int deviceId, int smallRumble, int bigRumble);
// set controller player number indicator (not all controllers have a number indicator which can be set, but that just means nothing will be done when this is called -- no harm)
JOY_SHOCK_API void JslSetPlayerNumber(int deviceId, int number);
